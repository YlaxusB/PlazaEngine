...
void PickingTexture::GenerateTexture() {
	glDisable(GL_BLEND);
	Application::Get()->pickingTexture->enableWriting();
	...
	glStencilMask(0xFF);
	Render(*Application::Get()->pickingShader);
	Application::Get()->pickingTexture->disableWriting();
	glEnable(GL_BLEND);
}
...
void PickingTexture::Render(Shader& shader) {
	...
	for (auto& entity : Application::Get()->activeScene->entities) {
