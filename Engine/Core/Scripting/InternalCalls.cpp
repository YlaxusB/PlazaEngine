...
//				}
//				else {
//					RenderGroup* newRenderGroup = new RenderGroup(Scene::GetActiveScene()->entities.at(uuid).GetComponent<MeshRenderer>()->mesh, Scene::GetActiveScene()->entities.at(uuid).GetComponent<MeshRenderer>()->renderGroup->materia//ls);
...
//	}
//
//	static void Collider_AddShapeMeshCall(uint64_t uuid, ColliderShape::ColliderShapeEnum shape, glm::vec3* vertices, int verticesSize, unsigned int* indices, int indicesSize, glm::vec3* normals, int normalsSize, glm::vec2* uvs, int uvsSi//ze) {
//		auto it = Scene::GetActiveScene()->colliderComponents.find(uuid);
//		if (it != Scene::GetActiveScene()->colliderComponents.end()) {
