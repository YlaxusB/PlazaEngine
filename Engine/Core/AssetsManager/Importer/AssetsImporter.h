...
namespace Plaza {
	...
	public:
		...
		static std::shared_ptr<Scene> ImportOBJ(AssetImported asset, std::filesystem::path outPath, Model& model, const AssetsImporterSettings& settings = AssetsImporterSettings{});
	...
	private:
		...
		//static Material* ObjModelMaterialLoader(const tinyobj_opt::material_t* tinyobjMaterial, const std::string materialFolderPath, std::unordered_map<std::string, uint64_t>& loadedTextures);
