...
namespace Plaza::Editor {
	...
	void ScriptManager::ReloadScriptsAssembly(Scene* scene, const std::string& dllPath) {
		...
		if (Mono::mScriptAssembly) {
			...
			for (uint64_t value : SceneView<CsScriptComponent>(scene)) {
				CsScriptComponent* component = scene->GetComponent<CsScriptComponent>(value);
				std::string scriptPath = component->scriptPath;
				uint64_t scriptUuid = component->mScriptUuid;
				component->mScriptUuid = scriptUuid;
	...
	void ScriptManager::ReloadScriptsAssembly(Scene* scene) {
		...
		if (scriptDllExists) {
#ifdef  GAME_REL
			ScriptManager::ReloadScriptsAssembly(scene, (Application::Get()->projectPath + "\\Binaries\\" + std::filesystem::path{ Application::Get()->activeProject->mAssetName }.stem().string() + ".dll").c_str());
#else
			ScriptManager::ReloadScriptsAssembly(scene, (Application::Get()->projectPath + "\\Binaries\\" + std::filesystem::path{ Application::Get()->activeProject->mAssetName }.stem().string() + "copy.dll").c_str());
#endif //  GAME_REL
		}
	...
	void ScriptManager::ReloadSpecificAssembly(Scene* scene, const std::string& scriptPath) {
		...
		if (script != Application::Get()->activeProject->scripts.end()) {
			std::map<uint64_t, std::map<std::string, std::map<std::string, Field*>>> allFields = FieldManager::GetAllScritpsFields(scene);
			...
			for (uint64_t value : SceneView<CsScriptComponent>(scene)) {
				CsScriptComponent* component = scene->GetComponent<CsScriptComponent>(value);
				std::string scriptPath = component->scriptPath;
				component->mScriptUuid = component->mScriptUuid;
				component->Init();
			}
	...
	std::unordered_map<uint64_t, std::map<std::string, std::unordered_map<std::string, uint32_t>>> ScriptManager::GetAllFields() {
		...
		std::unordered_map<uint64_t, std::map<std::string, std::unordered_map<std::string, uint32_t>>> fields;
		// FIX: Make it work again (did it ever worked?)
