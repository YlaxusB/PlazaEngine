...
namespace Engine {
	...
	std::vector<glm::mat4> ShadowsClass::GetLightSpaceMatrices()
	{
		...
		for (size_t i = 0; i < shadowCascadeLevels.size() + 1; ++i)
		{
			if (i == 0)
			{
				ret.push_back(getLightSpaceMatrix(Application->editorCamera->nearPlane - 3.0f, shadowCascadeLevels[i]));
			}
			else if (i < shadowCascadeLevels.size())
			{
				ret.push_back(getLightSpaceMatrix(shadowCascadeLevels[i - 1] - 3.0f, shadowCascadeLevels[i]));
			}
		}
		return ret;
	}
