...
namespace Plaza {
	...
	OpenGLRenderer* Bloom::mRenderer = nullptr; 
	GLuint Bloom::fbo = 0;
	...
	void Bloom::DrawBloom() {
		...
		glBindTexture(GL_TEXTURE_2D, mRenderer->bloomFrameBuffer->colorBuffer);
		glBindImageTexture(1, mRenderer->bloomFrameBuffer->colorBuffer, 0, GL_FALSE, 0, GL_READ_WRITE, GL_RGBA32F);
