...
namespace Plaza {
	...
	void VulkanShadows::InitializeBuffers(VulkanRenderer& renderer) {
		...
		imageInfo.format = renderer.FindDepthFormat();
		...
		//renderer.TransitionImageLayout(this->mShadowDepthImage, renderer.FindDepthFormat(), VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_IMAGE_ASPECT_DEPTH_BIT, 9);
		renderer.TransitionImageLayout(this->mShadowDepthImage, renderer.FindDepthFormat(), VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL, VK_IMAGE_ASPECT_DEPTH_BIT, 9);

		mShadowDepthImageViews.resize(9);
		for (unsigned int i = 0; i < 9; ++i) {
			VkImageViewCreateInfo viewInfo{};
			viewInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
			viewInfo.image = this->mShadowDepthImage;
			viewInfo.viewType = VK_IMAGE_VIEW_TYPE_2D_ARRAY;
			viewInfo.format = renderer.FindDepthFormat();
			viewInfo.subresourceRange.baseMipLevel = 0;
			viewInfo.subresourceRange.levelCount = 1;
			viewInfo.subresourceRange.baseArrayLayer = i;
			viewInfo.subresourceRange.layerCount = 9 - i;
	...
	void VulkanShadows::Init() {
		...
		mShadowsShader->Init(VulkanRenderer::GetRenderer()->mDevice, this->mRenderPass, Application->appSizes->sceneSize.x, Application->appSizes->sceneSize.y, this->mDescriptorSetLayout);
